...
const ICONS: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
  LayoutDashboard, Users, Shield, Trophy, MapPin, Award, Flag,
  AlertTriangle, FileText, TrendingUp, Settings,
};
...
export default function AdminSidebar() {
    ...
    <div className="flex h-full flex-col bg-gray-900">
      ...
      <div className="flex h-16 items-center border-b border-gray-700 px-5">
        <Link href="/admin" className="flex items-center gap-2">
          ...
          />
          <span className="text-lg font-bold text-white font-display">Admin</span>
        </Link>
      </div>
      ...
      <div className="border-t border-gray-700 p-3">
        ...
        <Link
          href="/dashboard"
          className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-green-400 hover:bg-gray-800"
        >
          Retour au site
        </Link>
        <button
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="fixed left-4 top-4 z-40 rounded-lg bg-gray-800 p-2 shadow-md lg:hidden">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="relative h-full w-64 shadow-xl">
            <button onClick={() => setMobileOpen(false)} className="absolute right-3 top-4 z-10 text-gray-400"><X size={20} /></button>
            {sidebar}
          </div>
        </div>
      )}
