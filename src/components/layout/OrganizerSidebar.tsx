...
import { Trophy, Globe, MessageCircle, LogOut } from "lucide-react";
import { useAuth } from "@/contexts/AuthContext";
import { ORGANIZER_GROUPED_NAV, isNavGroup, type NavItem, type NavEntry } from "@/config/navigation";
import SidebarNavGroup from "./SidebarNavGroup";

const ICONS: Record<string, React.ComponentType<{ size?: number; className?: string }>> = {
  Trophy, Globe, MessageCircle,
};
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-16 items-center border-b border-gray-200 px-5">
        <Link href="/organizer" className="flex items-center gap-2">
          ...
          <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs font-semibold text-amber-700">ORGA</span>
        </Link>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3">
        <div className="space-y-1">
          {ORGANIZER_GROUPED_NAV.map((entry: NavEntry) => {
            if (isNavGroup(entry)) {
              return (
                <SidebarNavGroup
                  key={entry.key}
