// Outils communs aux scripts de capture du tutoriel installation.
...
/** Un ordinateur plus étroit : les pages pleine largeur restent lisibles une fois imprimées. */
export const BUREAU = { viewport: { width: 1100, height: 800 }, deviceScaleFactor: 2 };
...
/** Un téléphone Android sous Chrome (le PHONE plus haut est un iPhone sous Safari). */
 ...
 * `proposition` : laisser KoppaFoot proposer l'installation (sinon on la fait
 * taire, elle masquerait les captures des autres guides).
 */
export async function open(profile, device = DESKTOP, { proposition = false } = {}) {
  ...
  await ctx.addInitScript((proposition) => {
    try {
      if (proposition) localStorage.removeItem("koppafoot:install-repousse");
      else localStorage.setItem("koppafoot:install-repousse", String(Date.now() + 1e11));
    } catch {}
  }, proposition);
...
export async function loginUI(page, email, password) {
  await go(page, "/login");
  // Profil déjà connecté : /login renvoie ailleurs.
...
/**
 * Ce que fait Chrome quand un site peut s'installer : l'évènement
 * `beforeinstallprompt`. Un navigateur piloté ne l'émet jamais ; on le
 * rejoue pour que KoppaFoot affiche ce qu'il affiche vraiment dans ce cas.
 * La fenêtre de Chrome qui suit, elle, n'est pas une page : elle est dessinée
 * en schéma (schemas.mjs).
 */
export async function simulerInstallable(page) {
  await page.evaluate(() => {
