...
/** Des comptes deviennent membres d'une équipe (ce que fait l'acceptation d'une candidature). */
export async function addMembers(teamId, uids) {
  await db.doc(`teams/${teamId}`).update({ member_ids: FieldValue.arrayUnion(...uids), updated_at: FieldValue.serverTimestamp() });
}

/** Les dossards des joueurs avec compte, tels que le manager les enregistre. */
export async function setSquadNumbers(teamId, numbers) {
  const patch = Object.fromEntries(Object.entries(numbers).map(([uid, n]) => [`squad_numbers.${uid}`, n]));
  await db.doc(`teams/${teamId}`).update({ ...patch, updated_at: FieldValue.serverTimestamp() });
}

/** Le créneau d'entraînement de chaque semaine (Paramètres de l'équipe). */
export async function setTrainingSchedule(teamId, slots) {
  await db.doc(`teams/${teamId}`).update({ training_schedule: slots, updated_at: FieldValue.serverTimestamp() });
}

/** Une séance ponctuelle, comme « Créer un entraînement » : tous les membres sont conviés. */
export async function createTraining({ teamId, title, date, time, location, description }) {
  const t = (await db.doc(`teams/${teamId}`).get()).data();
  const attendees = (t.member_ids ?? []).filter((id) => id !== t.manager_id).map((player_id) => ({ player_id, status: "pending" }));
  await db.collection("trainings").add({
    team_id: teamId, manager_id: t.manager_id, title, date, time, location,
    ...(description ? { description } : {}), attendees,
    created_at: FieldValue.serverTimestamp(), updated_at: FieldValue.serverTimestamp(),
  });
}

/** L'uid d'un compte, par son email. */
export async function uidOf(email) {
  return (await auth.getUserByEmail(email)).uid;
}

/** Un appel à l'API de l'application, au nom d'un compte (gestes joués en coulisses). */
export async function api(email, password, method, url, corps) {
  const tok = await idToken(email, password);
...
/** Le premier terrain d'un gérant. */
export async function terrainDe(email) {
  const u = await auth.getUserByEmail(email);
  const snap = await db.collection("venues").where("owner_id", "==", u.uid).get();
  return snap.docs[0]?.id ?? null;
}

/** Un compte sans rôle, prêt à se connecter (email vérifié). */
export async function compteSimple({ prenom, nom, email, mdp, ville = "Lomé" }) {
  let u;
  try { u = await auth.getUserByEmail(email); }
  catch { u = await auth.createUser({ email, password: mdp, displayName: `${prenom} ${nom}`, emailVerified: true }); }
  await db.collection("users").doc(u.uid).set({
    email, first_name: prenom, last_name: nom, user_type: "user", location_city: ville,
    phone: null, is_active: true, auth_providers: ["email"],
    created_at: FieldValue.serverTimestamp(), updated_at: FieldValue.serverTimestamp(),
  }, { merge: true });
  return u.uid;
}
