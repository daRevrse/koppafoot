// Outils communs aux scripts de capture du tutoriel organisateur.
...
export async function mark(page, targets) {
  ...
  for (const t of list) {
    ...
    if (box && t.union) {
      const b2 = await t.union.boundingBox();
      if (b2) {
 ...
 */
export async function region(page, name, loc, { marks, margin = 44, extraBottom = 0, hideHeader = true, maxHeight = 5000 } = {}) {
  await settle(page, 400);
  ...
  const height = Math.min(maxHeight, b.height + margin * 2 + extraBottom);
  const file = path.join(OUT, name + ".jpg");
